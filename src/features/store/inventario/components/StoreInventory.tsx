...
import { useStoreInventoryContext } from "../hooks/useStoreInventoryContext";
import { type FormFilterInventory } from "../types/storeInventory";
import { filterInventario } from "../utils/inventarioFilters";
import { initialFilteredFormProduct } from "../../utils/initialValues";
import { useStorePendEntriesContext } from "../../entradas_pendientes/hooks/useStorePendEntriesContext";
...
export const StoreInventory = () => {
  ...
  const { inventario, getStoreInventory } = useStoreInventoryContext();

  // Contexto de entradas pendientes
  const { pendEntries, getPendingEntries } = useStorePendEntriesContext();
          ...
          <table className="table-fixed min-w-full max-w-full border border-gray-200 rounded-lg shadow-sm text-sm text-left">
            <thead className="bg-[#E8B7BA] text-white sticky top-0 z-10">
              <tr>
                ...
                <th className="px-4 py-3 text-[#82385D]">Inventario</th>
              ...
              {filteredInventario.map((item, index) => (
                <tr key={index}>
                  <td className="px-4 py-2">
                    {item.tiene_lote_disponible && <BsBoxes />}
                  </td>
