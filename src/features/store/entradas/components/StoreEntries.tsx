...
export const StoreEntries = () => {
  ...
  const { entries, getStoreEntry } = useStoreEntriesContext();
        ...
        <div className="min-w-full max-w-full overflow-y-auto">
          <table className="table-fixed min-w-full max-w-full border border-gray-200 rounded-lg shadow-sm text-sm text-left">
            <thead className="bg-[#E8B7BA] text-white sticky top-0 z-10">
              <tr>
                <th className="px-4 py-3 text-[#82385D]">N° Factura</th>
            ...
            <tbody>
              {filteredEntries.map((item, index) => (
                <tr key={index}>
                  <td className="px-4 py-2">{item.nro_factura}</td>
                  <td className="px-4 py-2">
                    {dayjs(item.fecha_entrada).format("YYYY-MM-DD HH:mm:ss")}
                  </td>
