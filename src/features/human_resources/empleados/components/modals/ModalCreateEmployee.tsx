...
import { initialFormNewEmployee } from "../../utils/initialValues";
...
export default function ModalCreateEmployee() {
  ...
  const { employees, saveEmployee, getEmployees } = useEmployeeContext();
  ...
  const handlerCreateEmployee = async () => {
    // Asignar a una constante los datos del nuevo empleado
    const new_employee = dataEmployee;

    // Validar que todos los campos estén llenos
    const isValid = Object.entries(new_employee)
      .filter(([key]) => key !== "dias_vacaciones")
      .filter(([key]) => key !== "auxilio_transporte")
      .every(([, value]) => {
        if (typeof value === "boolean") return true; // false es válido
  ...
  useEffect(() => {
    setDataEmployee(initialFormNewEmployee);
  }, [open]);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button className="flex items-center justify-center bg-[#82385D] font-medium text-[#E8B7BA] h-auto cursor-pointer p-3 rounded-xl">
          <IoPersonAddOutline className="mr-2" size={20} color="#E8B7BA" />
          Crear Empleado
        </button>
      </DialogTrigger>
      ...
      <DialogContent className="!w-[95vw] !h-[95vh] !max-w-none bg-white border-none text-[#484848]">
        ...
        <div className="flex items-center justify-around flex-wrap overflow-y-auto">
          ...
          {errorMessage && (
            <span className="bg-[#c2361b] text-white font-semibold w-[100%] p-2 px-4 text-center rounded-xl">
